...
#if defined(NDEBUG)
# error "Achilles cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x254ed83fae4aec59ca2b21dbeee94790f3d20bf0de9268f9289bfa77d75fa653");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Achilles: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Achilles Signed Message:\n";
...
{
    // Achilles: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    // Subsidy is cut in half every 130625 blocks
    nSubsidy >>= (nHeight / 130625);
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Achilles: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Achilles: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        // Genesis Block:
        // CBlock(hash=12a765e31ffd4059bada, PoW=0000050c34a64b415b6b, ver=1, hashPrevBlock=00000000000000000000, hashMerkleRoot=97ddfbbae6, nTime=1516254025, nBits=1e0ffff0, nNonce=0, vtx=1)
        ...
        // Genesis block
        const char* pszTimestamp = "Apple to pay 38bn on foreign cash pile";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 80 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04c9e9dca07d6926aac1c13962453b065ec10bcbe93db92024a2381625e03e259a0a1ac5d5ab4b3448a095f117f6a3e183c51579e06dd42a5bdb17926db2908642") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1516254025;
        ...
        assert(block.hashMerkleRoot == uint256("0x1d78b210d6c2a2a3152f3539a05dd1870d21ac34b0ab9bb63a188fd2321c8081"));
...
unsigned char pchMessageStart[4] = { 0x9e, 0x5e, 0xec, 0xf6 }; // Achilles: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("AchillesMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("AchillesMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static AchillesMiner(CWallet *pwallet)
{
    printf("AchillesMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("achilles-miner");
    ...
    try { loop {
        ...
        printf("Running AchillesMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("AchillesMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&AchillesMiner, pwallet));
}
